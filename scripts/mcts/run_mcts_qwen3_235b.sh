...
NUM_GPUS=4  # 235B FP8 needs 4 GPUs minimum (with 48GB+ VRAM each)
NUM_PROCESSES=10
dataset="web_grounding" # sat2, web_grounding, vstar, web_action
PORT=9002 # port for vllm server (different from qwen72b to avoid conflicts)
...
if [ "$dataset" == "sat2" ]; then
  ...
  SAVE_TAG="MCTS_OSATLAS_QWEN3_235B"
  JUDGE="point_in_bbox"
  ...
  SAVE_TAG="MCTS_VSTAR_QWEN3_235B"
  JUDGE="string_match"
...
"
  ...
  SAVE_TAG="MCTS_WEB_ACTION_QWEN3_235B"
  JUDGE="web_action"
fi
...
#    If not running, start serve_qwen3_thinking.sh in the background.
#    Then wait for it to become reachable.
# ---------------------------------------------------------------------
if [ "$ACTOR_MODEL" == "qwen_vllm" ]; then
  # Simple check: see if something is already listening on port
  ...
  if lsof -Pi :${PORT} -sTCP:LISTEN -t >/dev/null 2>&1 || pgrep -f "vllm serve" >/dev/null 2>&1; then
    ...
    echo "[INFO] Starting vllm server via serve_qwen3_thinking.sh ..."
  ...
  # ----------------------------------------------------------------
  # 2) Wait for the vllm server to be reachable on the specified port
  #    We'll try up to 360 times, sleeping 10 seconds each time.
  # ----------------------------------------------------------------
  echo "[INFO] Please wait for vllm server to be responsive..."
  for i in {1..360}; do
    if curl --max-time 2 -s -o /dev/null -H "Authorization: Bearer qwen3" http://localhost:${PORT}/v1/models; then
