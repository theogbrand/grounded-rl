...
class Qwen3_VL(lmms):
    """
    Wraps a Qwen3-VL model for single-step generation 
    of 'thoughts' or final answers. Loaded one instance per Accelerate process.
    
    Note: Qwen3-VL uses the same architecture as Qwen2.5-VL, so we use
    Qwen2_5_VLForConditionalGeneration which is compatible with Qwen3 models.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Qwen3-VL uses the same architecture as Qwen2.5-VL
    ...
    def _encode_image(self, image: Image.Image) -> str:
        """
        Converts a PIL Image to base64-encoded JPEG for Qwen3-VL's 'image' content.
        """
        buffer = BytesIO()
    ...
    def generate_single_thought(
        ...
        # Build messages for Qwen3-VL (same format as Qwen2.5-VL)
        messages = [
            {
                "role": "system",
        ...
        # Qwen3-VL expects a list of entire conversation(s). 
        # We'll wrap our single conversation in a list:
        conversations = [messages]

        # Convert conversation to text that Qwen3-VL uses, while also extracting image/video
